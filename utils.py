...
def load_data(data_dir=None):
    if data_dir is None:
        data_dir = os.environ.get('BCI_DATA_DIR', '/content/drive/MyDrive/BCI/data/npz_files')
    ...
    for i in range(1,10):
        data_path = os.path.join(data_dir, 'A{:02d}T.npz'.format(i))
        subject = 'subject{:02d}'.format(i)
        subjectData[subject] = np.load(data_path)

        data_path = os.path.join(data_dir, 'A{:02d}E.npz'.format(i))
        subject = 'subject{:02d}'.format(i)
...
def subject_to_arrays(npz_train, npz_eval, mat_path):
    """Preprocess one subject's data, returning numpy arrays."""
...
def subject_to_tensors(npz_train, npz_eval, mat_path):
    """Preprocess one subject's data, returning PyTorch tensors."""
